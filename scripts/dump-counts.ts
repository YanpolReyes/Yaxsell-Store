...
async function main() {
  ...
  for (const col of collections) {
    try {
      const r = await fetch(`${BASE}/databases/${DB}/collections/${col}/documents?queries[]=${encodeURIComponent(JSON.stringify({ method: 'limit', values: [1] }))}`, { headers: h });
      const d = await r.json();
      counts[col] = d.total ?? 0;
    } catch {
      counts[col] = -1;
    }
  }
  ...
  let buckets: any[] = [];
  try {
    const r = await fetch(`${BASE}/storage/buckets`, { headers: h });
    const d = await r.json();
    buckets = d.buckets || [];
  } catch {}
  ...
  console.log('=== BUCKETS ===');
  console.log(JSON.stringify(buckets.map((b: any) => ({ id: b.$id, name: b.name, fileEncryption: b.fileEncryption, maximumFileSize: b.maximumFileSize })), null, 2));
}
