...
export default function AnimHeart({ filled = false, size = 22, className = '' }: AnimHeartProps) {
  ...
  return (
    <div className={className} style={{ position:'relative', width:s, height:s, display:'flex', alignItems:'center', justifyContent:'center' }}>
      {animating && <div style={{ position:'absolute', width:s*.7, height:s*.7, borderRadius:'50%', border:'2px solid #ec4899', animation:'ahR .6s ease-out forwards', pointerEvents:'none', zIndex:1 }}/>}
      {animating && Array.from({length:PC},(_,i)=>{
        const ps=Math.max(2.5,s*.12);
        return <div key={i} style={{ position:'absolute', width:ps, height:ps, borderRadius:'50%', background:i%3===0?'#f472b6':i%3===1?'#ec4899':'#f9a8d4', left:'50%', top:'50%', marginLeft:-ps/2, marginTop:-ps/2, animation:`ap${i} .5s ${i*25}ms ease-out forwards`, pointerEvents:'none', zIndex:1 }}/>;
      })}
      <svg viewBox="0 0 24 24" width={s} height={s} style={{ animation:animating?'ahB .55s cubic-bezier(.34,1.56,.64,1) forwards':'none', filter:filled?'drop-shadow(0 1px 3px rgba(236,72,153,.35))':'none', transition:'filter .3s', position:'relative', zIndex:2 }}>
        <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" fill={filled?'#ec4899':'none'} stroke={filled?'#ec4899':'#f9a8d4'} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" style={{ transition:filled&&!animating?'fill .25s,stroke .25s':'none', transformOrigin:'12px 12px', animation:animating?'ahF .35s ease forwards':'none' }}/>
