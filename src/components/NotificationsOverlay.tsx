    ...
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)' }} onClick={onClose}>
        <div style={{ ...panelStyle, padding: 32, textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
          ...
          <button type="button" onClick={() => { onClose(); router.push('/login'); }} style={{ padding: '10px 24px', background: `linear-gradient(135deg,${PINK},#db2777)`, color: '#fff', border: 'none', borderRadius: 999, fontWeight: 700, cursor: 'pointer' }}>
            Iniciar sesión
          </button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div style={panelStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 16px', borderBottom: '1px solid #fce7f3', background: 'linear-gradient(135deg, #fef2f8, #fdf2f8)' }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: read ? '#fff' : 'linear-gradient(135deg,#fef2f8,#fff)',
