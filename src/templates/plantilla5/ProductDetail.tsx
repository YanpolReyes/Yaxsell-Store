...
export default function ProductDetail({ previewProductId }: { previewProductId?: string }) {
  ...
  const applyYaxsellData = () => {
    ...
    const effectivePrice = isWholesaleQty ? displayProduct.WHOLESALEPRICE! : displayPrice;
    ...
    const originalVal = priceResolved.originalPrice || Math.round((displayProduct.PRICE || 0) / 0.8);
    ...
    root.querySelectorAll('accordion-component, .accordion--standard, .block-accordion').forEach(accordion => {
      ...
      if (isMainAccordion) {
        ...
        if (row1) {
          ...
          if (contentEl) {
            contentEl.innerHTML = `<div style="white-space: pre-line; font-size: 14px; line-height: 1.6; color: #4b5563; padding: 10px 0;">En Yaxsell nos esmeramos por entregarte la mejor experiencia de compra. Todos nuestros productos cuentan con garantía de calidad y soporte post-venta personalizado.</div>`;
          }
        }
