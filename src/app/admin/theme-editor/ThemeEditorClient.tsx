        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {CM_NAV.map(m => (
            <button key={m.id} onClick={() => applyCmNav(m)}
              ...
              <MiniCmNav m={m} />
              <div style={{ padding: '3px 6px', background: s.cmNavModel === m.id ? '#fff7ed' : '#fff', textAlign: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#374151' }}>{m.label}</div>
                <div style={{ fontSize: 8, color: '#9ca3af', lineHeight: 1.2 }}>{m.desc}</div>
              </div>
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {(['left', 'center'] as const).map(p => (
            <button key={p} onClick={() => onUpdate({ cmNavLogoPosition: p })}
              style={{ padding: '6px 0', fontSize: 11, fontWeight: 600, border: `2px solid ${s.cmNavLogoPosition === p ? '#F97316' : '#e5e7eb'}`, borderRadius: 6, background: s.cmNavLogoPosition === p ? '#fff7ed' : '#fff', cursor: 'pointer', color: '#374151' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
          {([['none','Sin'], ['sm','S'], ['md','M'], ['lg','XL']] as const).map(([v, l]) => (
            <button key={v} onClick={() => onUpdate({ cmNavShadow: v })}
              style={{ padding: '5px 0', fontSize: 10, fontWeight: 600, border: `1.5px solid ${s.cmNavShadow === v ? '#F97316' : '#e5e7eb'}`, borderRadius: 4, background: s.cmNavShadow === v ? '#fff7ed' : '#fff', cursor: 'pointer', color: '#374151' }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {CM_HERO.map(m => (
            <button key={m.id} onClick={() => applyCmHero(m)}
              ...
              <MiniHero m={m} />
              <div style={{ padding: '3px 6px', background: s.cmHeroModel === m.id ? '#fff7ed' : '#fff', textAlign: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#374151' }}>{m.label}</div>
                <div style={{ fontSize: 8, color: '#9ca3af', lineHeight: 1.2 }}>{m.desc}</div>
              </div>
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
          {(['left', 'center', 'right'] as const).map(a => (
            <button key={a} onClick={() => onUpdate({ cmHeroAlign: a })}
              style={{ padding: '6px 0', fontSize: 11, fontWeight: 600, border: `2px solid ${s.cmHeroAlign === a ? '#F97316' : '#e5e7eb'}`, borderRadius: 6, background: s.cmHeroAlign === a ? '#fff7ed' : '#fff', cursor: 'pointer', color: '#374151' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {CM_FOOTER.map(m => (
            <button key={m.id} onClick={() => applyCmFtr(m)}
              ...
              <MiniFtr m={m} />
              <div style={{ padding: '3px 6px', background: s.cmFooterBg === m.bg ? '#fff7ed' : '#fff', textAlign: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#374151' }}>{m.label}</div>
                <div style={{ fontSize: 8, color: '#9ca3af', lineHeight: 1.2 }}>{m.desc}</div>
              </div>
            </button>
