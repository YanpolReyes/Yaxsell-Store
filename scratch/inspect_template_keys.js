const sdk = require('node-appwrite');
const c = new sdk.Client()
  .setEndpoint('https://nyc.cloud.appwrite.io/v1')
  .setProject('6a0a4e8d0032177f3f90')
  .setKey('standard_');
const db = new sdk.Databases(c);

async function run() {
  try {
    const res = await db.listDocuments('6a0a58ca001798410d86', 'sequences', [
      sdk.Query.limit(100)
    ]);
    console.log('ALL SEQUENCE KEYS:');
    res.documents.forEach(doc => {
      console.log(`- ${doc.key}: ${doc.value} ($id: ${doc.$id})`);
    });
  } catch(e) {
    console.error('Error listing sequences:', e);
  }
}
run();
